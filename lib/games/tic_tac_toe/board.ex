defmodule Games.TicTacToe.Board do
  ...
  def row(board, row_number) do
     for {[row, _col] = position, mark} <- board, row == row_number, into: %{}, do: {position, mark}
  end

  def col(board, col_number) do
    for {[_row, col] = position, mark} <- board, col == col_number, into: %{}, do: {position, mark}
  end
  ...
  def right_left_diagonal(%{size: size} = board) do
    for {[row,col] = position, mark} <- board, col == (size - row) + 1, into: %{}, do: {position, mark}
  end
end
